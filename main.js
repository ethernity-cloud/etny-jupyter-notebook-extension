 ...
 *
 */
define(["require", "base/js/namespace", "base/js/dialog", "https://cdnjs.cloudflare.com/ajax/libs/ethers/5.6.8/ethers.umd.min.js", "https://unpkg.com/ipfs-http-client/dist/index.min.js"], function (requirejs, Jupyter, dialog, ethers, ipfsHttpClient) {
    ...
    const initialize = async () => {
        ...
        etnyContract = new ethers.Contract(smartContractAddress, abi, signer);
    ...
    const callContractAddDORequest = async (metadata1, metadata2, metadata3) => {
        const cpu = 1, memory = 1, storage = 40, bandwidth = 1, duration = 60, instances = 1, maxPrice = 0,
            imageHash = __imageHash;
        return await etnyContract._addDORequest(cpu, memory, storage, bandwidth, duration, instances, maxPrice, imageHash, metadata1, metadata2, metadata3);
    }
        ...
        try {
            ...
            for (let i = endBlockNumber; i >= startBlockNumber; i--) {
                ...
                for (const transaction of block.transactions) {
                    if (transaction.to === smartContractAddress && transaction.data) {
                        // console.log('helllllllllllo');
                        // const transactionInput = await etnyStakingContract.decode_function_input(transaction.input);
                        // const iface = new ethers.utils.Interface(['function addResultToOrder(uint256 orderItem, string _result)']);
                        // const x = iface.decodeFunctionData('_addResultToOrder', transaction.input);
                        // console.log(x);
                        //const transactionInput = ethers.utils.defaultAbiCoder.decode(["uint256", "string"], transaction.data);
